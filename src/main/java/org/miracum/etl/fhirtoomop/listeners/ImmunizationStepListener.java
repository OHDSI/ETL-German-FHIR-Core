...
@Slf4j
@Component
public class ImmunizationStepListener implements StepExecutionListener {
  ...
  public void beforeStep(StepExecution stepExecution) {
    if (bulkload.equals(Boolean.TRUE)) {
      if (!Strings.isNullOrEmpty(startSingleStep)) {
        ...
        stopWatch.stop();
        log.info(
            "Cleaned up all immunization data in [{}s]",
      ...
      dbMappings.setFindSnomedVaccineMapping(
          repositories.getSnomedVaccineRepository().getSnomedVaccineMap());
      dbMappings
  ...
  private void deleteAllImmunizationData(StepContribution contribution)
    ...
    Resource deleteMedicationAdministrationData =
        new ClassPathResource("single_step/single_step_clean_up_immunization_data.sql");
