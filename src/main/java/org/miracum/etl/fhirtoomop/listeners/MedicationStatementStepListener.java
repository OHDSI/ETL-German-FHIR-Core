...
/**
 * The MedicationStatementStepListener class describes activities to be performed before and after
 * the execution of the step for FHIR MedicationStatement resources.
 *
 * @author Elisa Henke
 * @author Yuan Peng
 */
@Slf4j
@Component
public class MedicationStatementStepListener implements StepExecutionListener {
  ...
  /**
   * Constructor for objects of the class MedicationStatementStepListener.
   *
  ...
  /**
   * Executes all activities which should take place before the step for FHIR MedicationStatement
  ...
  public void beforeStep(StepExecution stepExecution) {
    if (bulkload.equals(Boolean.TRUE)) {
      if (!Strings.isNullOrEmpty(startSingleStep)) {
        ...
        log.info(
            "Cleaned up all medication statement data in [{}s]",
  ...
  /**
   * Deletes all data from OMOP CDM that originates from FHIR MedicationStatement resources.
   *
  ...
  private void deleteAllMedicationStatementData(StepContribution contribution)
    ...
    Resource deleteMedicationStatementData =
        new ClassPathResource("single_step/single_step_clean_up_medication_statement_data.sql");

    executeSqlScripts.executeSQLScript(deleteMedicationStatementData);
  }

  /**
   * Executes all activities which should take place after the step for FHIR MedicationStatement
