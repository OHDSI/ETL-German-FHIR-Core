...
@Slf4j
@Component
public class ConsentStepListener implements StepExecutionListener {
  ...
  public void beforeStep(StepExecution stepExecution) {
    ...
    if (bulkload.equals(Boolean.TRUE)) {
      if (!Strings.isNullOrEmpty(startSingleStep)) {
        ...
        stopWatch.start();
        try {
          deleteAllConsentData(stepExecution.createStepContribution());
        ...
        log.info(
            "Cleaned up all consent data in [{}s]",
   ...
   * @throws SQLException
   */
  private void deleteAllConsentData(StepContribution contribution)
      throws SQLException, IOException {
    ExecuteSqlScripts executeSqlScripts = new ExecuteSqlScripts(dataSource, contribution);
    Resource deleteConsentData =
        new ClassPathResource("single_step/single_step_clean_up_consent_data.sql");

    executeSqlScripts.executeSQLScript(deleteConsentData);
  }
